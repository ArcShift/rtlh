...
class ProgressKondisi extends Component {
	...
	componentWillMount() {
		this.init();
		DeviceEventEmitter.addListener('refreshKondisi', (e)=>{ this.find(true) })
	}
	...
	filter = async(tahun, kegiatan) => {
		if(this.state.idGroup){
			var url = api.url + '/monitoring/kondisi/' + this.state.idGroup + '/' + this.state.idUser;
	...
	detail = async(data) => {
		...
		AsyncStorage.removeItem('kondisiList');
		this.setState({loading: true});
		
		try {
			var url = api.url + '/monitoring/detail_kondisi/' + data.id_penerima;
			...
			.then((responseJson) => {
				...
				if(responseJson.hasOwnProperty('code')){
					if(responseJson.code == '200'){
						...
						value = AsyncStorage.setItem('kondisiList', JSON.stringify(responseJson.data)).then(
							() => { this.setState({loading: false}); this.props.navigation.navigate('ListKondisi', {idUser: this.state.idUser, idGroup: this.state.idGroup, base: data}) }
						);
		...
		return (
			...
			<Container>
				...
				<Content>
					...
					{
						(this.state.data && this.state.data.length == 0 && this.state.kegiatanSelected && !this.state.loading) &&
...
export default ProgressKondisi;
