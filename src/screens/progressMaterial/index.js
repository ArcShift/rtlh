...
class ProgressMaterial extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true, 
	...
	componentWillMount() {
		this.init();
		DeviceEventEmitter.addListener('refreshMaterial', (e)=>{ this.find(true) })
	}
	...
	filter = async(tahun, kegiatan) => {
		if(this.state.idGroup){
			var url = api.url + '/monitoring/material/' + this.state.idGroup + '/' + this.state.idUser;
	...
	detail = async(data) => {
		...
		try {
			var url = api.url + '/monitoring/detail_material/' + data.id_penerima;
			...
			.then((responseJson) => {
				...
				if(responseJson.hasOwnProperty('code')){
					if(responseJson.code == '200'){
						...
						value = AsyncStorage.setItem('materialList', JSON.stringify(responseJson.data)).then(
							() => { this.setState({loading: false}); this.props.navigation.navigate('ListMaterial', {idUser: this.state.idUser, idGroup: this.state.idGroup, base: data}) }
						);
