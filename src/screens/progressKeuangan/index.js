...
class ProgressKeuangan extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true, 
	...
	filter = async(tahun, kegiatan) => {
		if(this.state.idGroup){
			var url = api.url + '/monitoring/keuangan/' + this.state.idGroup + '/' + this.state.idUser;
	...
	detail = async(data) => {
		...
		AsyncStorage.removeItem('materialList');
		this.setState({loading: true});
		
		try {
			var url = api.url + '/monitoring/detail_keuangan/' + data.id_penerima;
			...
			.then((responseJson) => {
				...
				if(responseJson.hasOwnProperty('code')){
					if(responseJson.code == '200'){
						...
						value = AsyncStorage.setItem('keuanganList', JSON.stringify(responseJson.data)).then(
							() => { this.setState({loading: false}); this.props.navigation.navigate('ListKeuangan', {idUser: this.state.idUser, idGroup: this.state.idGroup, base: data}) }
						);
			...
			<Container>
				...
				<Content>
					...
					
					{
						...
						<View style={{paddingHorizontal: 20, paddingVertical: 30}}>
						<Text style={{textAlign: 'center', color: 'red'}}>
							Silakan input Progress 0% melalui Website Dashboard
						</Text>
						</View>
					}
				</Content>
...
export default ProgressKeuangan;
